...
function DATA_NOT_FOUND(INFORMATION, T_ID) {
    ...
    new toBC("companyb").AutoPushInBlockchain(getkey).then((result) => {
    })
        .catch((error) => {
...
Fabric_Client.newDefaultKeyValueStore({
    ...
    return fabric_client.getUserContext('companyb', true);
}).then((user_from_store) => {
    if (user_from_store && user_from_store.isEnrolled()) {
        console.log('Successfully loaded companyb from persistence');
        member_user = user_from_store;
    } else {
        throw new Error('Failed to get companyb.... run registerUser.js');
    }
    ...
    fabric_client.queryChannels(peer).then((name) => {
        ...
        channelArray.forEach(channel => {
            ...
            ChannelEventArray.push(channel.newChannelEventHub("localhost:9051"))
        })
        ...
        ChannelEventArray.forEach(ChannelEvent => {
            var ChainCodeEvent = ChannelEvent.registerChaincodeEvent(chaincodeid, chaincodeEventName,
                async (event, block_num, txnid, status) => {
                    ...
                    var keyprivate = await db.DBreadprivate("companyb", "CompanyData", "companyb")   //// อันนี้ต้องทำของใครของมัน อันนี้ของโลตัส
                    // var pemFile = path.resolve(__dirname,`../../../controller/companyb/private_key.pem`)
                                            ...
                                            if (INFORMATION.TYPE == "succes") {
                                                ...
                                                if (INFORMATION.VERIFY == "Verify") {
                                                    if (INFORMATION.TYPE == "INVOICE") {
                                                        ///////////INVOICE
                                                        try {
                                                            var Verify = await db.DBread("companyb", INFORMATION.TYPE, `${INFORMATION.TYPE}_BODY|` + INFORMATION.INVOICE_KEY)
                                                            var SALT = await db.DBread("companyb", "PO", `PO_SALT|` + Verify.PO_KEY)
                                                        } catch (error) {
                                                        ...
                                                        try {
                                                            if (INFORMATION.TO == Verify.TO && INFORMATION.FROM == Verify.FROM && INFORMATION.TYPE == Verify.TYPE
                                                                ...
                                                                var INFO = "PO"
                                                                var PO = await db.DBread("companyb", INFO, `${INFO}_BODY|` + INFORMATION.PO_KEY)
                                                                var SALT = await db.DBread("companyb", INFO, `${INFO}_SALT|` + INFORMATION.PO_KEY)
                                                                var getkey = {
                                                                    FROM: "companyb",
                                                                    BANK: INFORMATION.BANK,
                                                                    PO: PO,
                                                                ...
                                                                var functionName = "eventcompanyb"
                                                        ...
                                                        /////////////PO
                                                        try {
                                                            var Verify = await db.DBread("companyb", INFORMATION.TYPE, `${INFORMATION.TYPE}_BODY|` + INFORMATION.PO_KEY)
                                                            var SALT = await db.DBread("companyb", "PO", `PO_SALT|` + Verify.PO_KEY)
                                                        } catch (error) {
                                                            // DATA_NOT_FOUND(INFORMATION)
                                                        }
